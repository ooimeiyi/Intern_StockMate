package com.example.intern_stockmate.ui.hourlySales
...
fun HourlySalesScreenContainer(viewModel: HourlySalesViewModel = viewModel()) {
    val state by viewModel.hourlySalesState.collectAsState()
    val selectedTabIndex = viewModel.selectedTabIndex.intValue

    val rawData = (state as? HourlySalesUiState.Success)?.data.orEmpty()
    val processedData = remember(rawData, selectedTabIndex) {
        HourlySales.mergeWithFullDay(rawData).map { sales ->
        ...
        when (state) {
            ...
            is HourlySalesUiState.Error -> {
                Box(
                    ...
                    Text(
                        text = (state as HourlySalesUiState.Error).message,
            ...
            is HourlySalesUiState.Success -> Unit
        }
    }
}

@Composable
private fun HourlyTabHeader(selectedTabIndex: Int, onTabSelected: (Int) -> Unit) {
        ...
        tabs.forEachIndexed { index, title ->
            ...
            Box(
                modifier = Modifier
                    ...
                    .then(
                        if (isSelected) {
                            Modifier.background(Color.White, RoundedCornerShape(12.dp)).padding(2.dp)
                        } else {
                            Modifier
                        }
                    )
...
@Composable
private fun HourlySalesScreen(salesData: List<HourlySales>, selectedTabIndex: Int) {
    ...
    Column(
        ...
        DetailedLogSection(data = salesData, selectedTabIndex = selectedTabIndex)
    }
}

@Composable
private fun SalesTrendCard(
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Text("Hourly Sales Trend", fontWeight = FontWeight.Bold, fontSize = 20.sp, color = Color.Black)
            ...
            Box(modifier = Modifier.height(300.dp).fillMaxWidth()) {
                Canvas(
                    ...
                    data.forEachIndexed { index, sales ->
                        ...
                        if (sales.hour in listOf(0, 4, 8, 12, 16, 20)) {
                            val hourLabel = "%02d:00".format(sales.hour)
                            val textLayout = textMeasurer.measure(hourLabel, xAxisLabelStyle)
                            drawText(
                                textMeasurer,
                                hourLabel,
                ...
                if (activeIndex != -1 && activeIndex < data.size) {
                    ...
                    val popupWidth = 160
                    val popupHeight = if (selectedTabIndex == 0) 120 else 70
                    ...
                    Popup(offset = IntOffset(popupX, popupY)) {
                        Surface(shadowElevation = 8.dp, shape = RoundedCornerShape(8.dp), color = Color.White) {
                            Column(modifier = Modifier.padding(8.dp)) {
                                Text("%02d:00".format(active.hour), fontSize = 15.sp, fontWeight = FontWeight.Bold, color = Color.Black)
...
@Composable
private fun HourlyChartLegend(color: Color, label: String, amount: Double, modifier: Modifier = Modifier) {
...
@Composable
fun DetailedLogSection(data: List<HourlySales>, selectedTabIndex: Int) {
