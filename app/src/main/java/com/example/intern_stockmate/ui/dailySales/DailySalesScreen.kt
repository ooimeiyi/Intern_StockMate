package com.example.intern_stockmate.ui.dailySales
...
import com.example.intern_stockmate.viewModel.DailySalesUiState
import com.example.intern_stockmate.viewModel.DailySalesViewModel
...
fun DailySalesScreenContainer(viewModel: DailySalesViewModel = viewModel()) {
    val state by viewModel.dailySalesState.collectAsState()
    val selectedTabIndex = viewModel.selectedTabIndex.intValue
    val successState = state as? DailySalesUiState.Success
    val yearMonth = successState?.yearMonth ?: YearMonth.now()
    val lastUpdate = successState?.lastUpdate.orEmpty()
    val rawData = successState?.data.orEmpty()

    val processedData = remember(rawData, selectedTabIndex, yearMonth) {
        DailySales.mergeWithFullMonth(rawData, yearMonth).map { sales ->
    ...
    Box(modifier = Modifier.fillMaxSize().background(Color(0xFFF5F7F9))) {
        Column(modifier = Modifier.fillMaxSize()) {
            DailyTabHeader(selectedTabIndex = selectedTabIndex, onTabSelected = viewModel::selectTab)
            DailySalesScreen(processedData, selectedTabIndex, yearMonth, lastUpdate)
        }

        when (state) {
            is DailySalesUiState.Loading -> {
                Box(
                    modifier = Modifier.fillMaxSize().background(Color.White.copy(alpha = 0.6f)),
                    contentAlignment = Alignment.Center
                ) { CircularProgressIndicator(color = Color.Red) }
            }
            is DailySalesUiState.Error -> {
                Box(
                    modifier = Modifier.fillMaxWidth().background(Color(0xFFFFCDD2)).padding(12.dp).align(Alignment.TopCenter)
                ) {
                    Text(
                        text = (state as DailySalesUiState.Error).message,
            ...
            is DailySalesUiState.Success -> Unit
        }
    }
}

@Composable
private fun DailyTabHeader(selectedTabIndex: Int, onTabSelected: (Int) -> Unit) {
        ...
        tabs.forEachIndexed { index, title ->
            ...
            Box(
                modifier = Modifier.weight(1f).height(40.dp)
                    .then(if (isSelected) Modifier.background(Color.White, RoundedCornerShape(12.dp)).padding(2.dp) else Modifier)
...
private fun DailySalesScreen(
    ...
    Column(
        modifier = Modifier.fillMaxSize().background(Color(0xFFF5F7F9)).pointerInput(Unit) {
            detectTapGestures {
                if (isLocked) {
                    isLocked = false
                    activeIndex = -1
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                Text(
                    text = yearMonth.month.getDisplayName(JavaTextStyle.FULL, Locale.US) + " ${yearMonth.year}",
                    fontWeight = FontWeight.Bold,
                    fontSize = 18.sp,
                    color = Color.Black
                )
                if (lastUpdate.isNotBlank()) {
                    Text(text = "Last update: $lastUpdate", fontSize = 12.sp, color = Color(0xFF5A6B82))
                }
            }
        }
        ...
        DailyDetailedLogSection(data = salesData, selectedTabIndex = selectedTabIndex)
    }
}
...
private fun DailyTrendCard(
    ...
    val xAxisLabelStyle = TextStyle(color = Color.Black, fontSize = 8.sp)
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Text("Daily Sales Trend", fontWeight = FontWeight.Bold, fontSize = 20.sp, color = Color.Black)
            ...
            Box(modifier = Modifier.height(300.dp).fillMaxWidth()) {
                Canvas(
                            ...
                            awaitPointerEventScope {
                                while (true) {
                                    ...
                                    if (event.type == PointerEventType.Move) {
                                        if (pos.y > chartBottom || pos.x < paddingLeft || pos.x > size.width) onActiveIndexChange(-1)
                                        else {
                    ...
                    if (activeIndex != -1) {
                        ...
                        drawRect(color = Color.Black.copy(alpha = 0.05f), topLeft = Offset(xPosHighlight, 0f), size = Size(barWidth + spaceBetween, chartHeight))
                    }
                    ...
                    data.forEachIndexed { index, sales ->
                        ...
                        if (sales.day == 1 || sales.day % 5 == 0 || sales.day == data.last().day) {
                            val dayLabel = "Day ${sales.day}"
                            val textLayout = textMeasurer.measure(dayLabel, xAxisLabelStyle)
                            drawText(textMeasurer, dayLabel, Offset(xPos - textLayout.size.width / 2, chartHeight + 12.dp.toPx()), xAxisLabelStyle)
                        }
                    }
                }
                    ...
                    Popup(offset = IntOffset(popupX, popupY)) {
                        Surface(shadowElevation = 8.dp, shape = RoundedCornerShape(8.dp), color = Color.White) {
                            Column(modifier = Modifier.padding(8.dp)) {
                                Text("Day ${active.day}", fontSize = 15.sp, fontWeight = FontWeight.Bold, color = Color.Black)
...
@Composable
private fun DailyChartLegend(color: Color, label: String, amount: Double, modifier: Modifier = Modifier) {
...
@Composable
fun DailyDetailedLogSection(data: List<DailySales>, selectedTabIndex: Int) {
